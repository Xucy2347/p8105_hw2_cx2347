...
mr_df = 
  ...
  # keep the dumpster obs that the weight is greater than 0
  filter(weight_tons > 0) |>
...
The data from `r min(pull(wheel_df, date))` to `r max(pull(wheel_df, date))` is collected.
For available data, 
the total weight of trash collected by Professor Trash Wheel is `r sum(pull(prof_df, weight_tons))` tons;
...
including the information of the Zillow Observed Rent Index in New York City between Janurary 2015 and August 2024.
...
The variables covered information related to 
  the geo information (eg. `region_name`, `state_fips` and so on), 
  the date when the ZORI is collected (eg. `x2015_01_31`), 
  and the size rank (`size_rank`).
...
&emsp; 1) some ZIP codes do not have a neighborhood or does not record the neightborhood in it. For example, for the area with ZIP codes of 10499, the neighborhood is listed as NA. So, it is unlikely that there are many properties rental or sales. Therefore, they will be excluded from the Zillow Observed Rental Dataset.\
