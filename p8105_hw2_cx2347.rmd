...
# convert the data set to a longer version
zillow_df = zillow_raw |>
  pivot_longer(
  ...
  arrange(size_rank, region_name, date)
...
The dataset `zillow_df` were combined from the `zip_df` and `zori_df`, 
including the information of the Zillow Observed Rent Index in New York City between January 2015 and August 2024.
There are `r nrow(zillow_df)` observation and `r ncol(zillow_df)` variables in the dataset.
The rental price of `r sum(is.na(pull(zillow_df, price)))` observations are not reported.
The variables covered information related to 
  ...
  the rental price for each month between January 2015 and August 2024 (`price`), 
  and the rank of sizes (`size_rank`).
The dataset is order by the `size_rank` (following the original dataset `zori_df`), `region_name` and `date`.
There are `r n_distinct(pull(zillow_df, region_name), na.rm = TRUE)` unique ZIP codes
  and `r n_distinct(pull(zillow_df, neighborhood), na.rm = TRUE)` unique neighborhoods in the dataset.
\
\
There are `r n_distinct(pull(zillow_non, region_name), na.rm = TRUE)` ZIP codes only appear in the ZIP code list but not in the Zillow Rental Price dataset, 
